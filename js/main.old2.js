    ...
    commands: [ // an array of commands to list in the menu
        ...
        },
        {
            ...
            select: function(){ 
				var nid = this.id();
				var del = -1;
				$(graph_elements.nodes).map(function(i,v) {
					if (v.data.id === nid) del = i;
				});
				if (del != -1) {
					console.log(graph_elements.edges);
					var edel = [];
					$(graph_elements.edges).map(function(i,v) {
					console.log("v.data " + v.data);
						if (v.data.source === del || v.data.target === del) edel.push(i);
					});
					console.log("edel " + edel);
					for (var k = 0; k < edel.length; k++)
						graph_elements.edges.splice(edel[k],1);			
						
					graph_elements.nodes.splice(del,1);			
					console.log(graph_elements.edges);
				}
				window.cy_monitoring.remove(this);
			}
        }
    ], 
...
var graph_context_menu_edge = {
    ...
    commands: [ // an array of commands to list in the menu
        ...
        },
        {
            content: 'Delete' ,
            select: function(){ 
//				var sid = this.data().streamid;
//				var del = -1;
//				$(graph_elements.edges).map(function(i,v) {
//					if (v.data.streamid === sid) del = i;
//				});
//				if (del != -1) graph_elements.edges.splice(del,1);			
				window.cy_monitoring.remove(this);
			}
        },
...
$(function() {
	...
	$('#monitoring_graph').cytoscapeCxtmenu( graph_context_menu_edge );   
//	$('#monitoring_graph').cytoscapeEdgehandles( edge_handle_defaults );

	var qmon_number_points = $('#main').innerWidth();
